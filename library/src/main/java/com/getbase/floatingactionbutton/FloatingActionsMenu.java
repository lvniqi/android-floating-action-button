...
public class FloatingActionsMenu extends ViewGroup {
    ...
    public void hideLabels(FloatingActionButton button) {
        button.setTitleHide(true);
        removeView(button.getLabelView());
    }

    public void showLabels(FloatingActionButton button) {
        button.setTitleHide(false);
        addView(button.getLabelView());
    ...
    public void collapse() {
        for (int i = 0; i < mButtonsCount; i++) {
            ...
            if (button.getSubButtons() != null) {
                for (View x : button.getSubButtons()) {
                    x.setVisibility(View.GONE);
                }
            }
            if (button.isTitleHide() == true) {
                button.setColorNormal(Color.rgb(0x00, 0x99, 0xcc));
                showLabels(button);
            }
        }
        if (mExpanded) {
